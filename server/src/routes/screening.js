...
router.post('/screenings', isLoggedIn, isPartnerOrAdmin, async (req, res) => {
    ...
    const { userId } = req;
    const screening = await Screenings.create({ theatre: theatreId, movie: movieId, price, showTimings, author: userId });
    res.json(ApiResponse.build(true, screening, 'Screening created successfully'));
});

router.get('/screenings/:movieId', async(req, res) => {
    const { movieId } = req.params;
    const screenings = await Screenings.find({ movie: movieId }).populate('theatre');
